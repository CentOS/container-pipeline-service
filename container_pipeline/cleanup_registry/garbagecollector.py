...
class GarbageCollector(object):
    ...
    def __init__(self, registry_host="127.0.0.1", registry_port="5000",
                 registry_secure=False, local_index=False,
                 index_git="https://github.com/centos/container-index",
                 verbose=True, collect=True):
        """
        Initialize the garbage collector object.

        :param registry_host: The ip or host name of registry to query.
                              Default is 127.0.0.1.
        :param registry_port: The port of the registry to query.
                              Default is 5000
        :param registry_secure: Is the registry secure or insecure
                                (https or http)
        ...
        :param verbose: If set then steps are displayed on
                        screen as they happen.
        :param collect: If set. then the garbage collection takes place,
                        else it does dry run.
        """
        self._verbose = verbose
        self._index_location = "./c_i"
        self._collect = collect
        if local_index:
            # If local index is set,
            # check if inex files are present at expected location.
    ...
    def _delete_mismatched(self):
        ...
        for container_full_name, tag_list in self.mismatched.iteritems():
            ...
            namespace = container_full_name.split(
                "/")[0] if "/" in container_full_name else container_full_name
    ...
    def collect(self):
            ...
            if "index_template" not in index_file:
                ...
                for entry in data["Projects"]:
                    ...
                    if not BuildTracker(container_tag).is_running():
                        self.index_containers[container_name].append(
                            desired_tag)
        ...
        for r_name, r_info in self._registry_info.tags.iteritems():
            ...
            for item1 in r_tags:
                ...
                if r_name in self.index_containers and \
                        item1 not in self.index_containers[r_name]:
