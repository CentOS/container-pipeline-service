...
class Validator(object):
    ...
    def __init__(self, context):
        ...
        Keyword arguments:
            context -- nuts_and_bolts.Context object that holds the global
            information such as environment, summary and Dependency Validator
        """
        self._success = True
    ...
    def run(self):
        """
        Runs the validator. returning True or False based on
        success or failure
        """
...
class IndexValidator(Validator):
    ...
    def __init__(self, context, index_file):
        ...
        if temp_index_data:
            if "Projects" in temp_index_data:
                ...
                self._success = False
                self._context.summary.global_errors.append(
                    "Projects absent in " + self._index_file)
        else:
            self._success = False
            self._context.summary.global_errors.append(
                "Malformed " +
                path.basename(self._index_file) + " : " + str(ex)
            )
        self._status_list = {
            "fail": [],
            "success": []
    ...
    def run(self):
        """Runs the IndexValidator, returning True or False based on
        success or failure"""
        raise NotImplementedError("Implement this method.")
    ...
    def run(self):
        ...
        for entry in self._index_data:
            ...
            self._summary_collector = SummaryCollector(
                self._context, self._file_name, entry)
            ...
            if constants.PREBUILD_SCRIPT in entry and entry[
                ...
                if not prebuild_path:
                    self._mark_entry_invalid(entry)
                    self._summary_collector.add_error("If pre-build script is \
                        specified, then prebuild-context should also be \
                        specified")

            # Checking app-id field
            if "app-id" not in entry or ("app-id" in entry and entry[
                    "app-id"] is None):
                self._mark_entry_invalid(entry)
            ...
            if "job-id" not in entry or ("job-id" in entry and entry[
                    "job-id"] is None):
                self._mark_entry_invalid(entry)
            ...
            # Check for git-url
            if "git-url" not in entry or ("git-url" in entry and entry[
                    "git-url"] is None):
                self._mark_entry_invalid(entry)
                self._summary_collector.add_error("Missing git-url")
            else:
                if "gitlab." in entry["git-url"] and not entry[
                        "git-url"].endswith(".git"):
                    self._mark_entry_invalid(entry)
                    self._summary_collector.add_error(
                        "Git urls from gitlab must end with .git, \
                         try {0}.git".format(entry["git-url"]))

            # Checking git-path
            if "git-path" not in entry or ("git-path" in entry and entry[
                    "git-path"] is None):
                self._mark_entry_invalid(entry)
                self._summary_collector.add_error("Missing git-path")

            # Check git-branch
            if "git-branch" not in entry or ("git-branch" in entry and entry[
                    "git-branch"] is None):
                self._mark_entry_invalid(entry)
                self._summary_collector.add_error("Missing git-branch")

            # Check target-file
            if "target-file" not in entry or ("target-file" in entry and entry[
                    "target-file"] is None):
                self._mark_entry_invalid(entry)
                self._summary_collector.add_error("Missing target-file")

            # Check desired-tag
            if "desired-tag" not in entry or ("desired-tag" in entry and entry[
                    "desired-tag"] is None):
                self._mark_entry_invalid(entry)
                self._summary_collector.add_error("Missing desired-tag")

            # Check for build-context
            # Ideally, build-context will be a compulsory field but for now im
            # just checking its None
            # TODO : Need to update this to make it compulsory
            if constants.BUILD_CONTEXT not in entry and not (
                    constants.BUILD_CONTEXT in entry and
                    entry[constants.BUILD_CONTEXT] is None):
                pass

            # Check notify-email
            if "notify-email" not in entry or (
                    "notify-email" in entry and entry["notify-email"] is None):
                self._mark_entry_invalid(entry)
            ...
            if "depends-on" not in entry:
                ...
                matcher = re.compile("^(([0-9a-zA-Z_-]+[.]{1})*([0-9a-zA-Z_-]+){1}[/]{1})?[0-9a-zA-Z_-]+[/]{1}[0-9a-zA-Z_-]+[:]{1}([0-9a-zA-Z_-]+\.?)+$"
                                     )
                for item in depends_on:
                    if not matcher.search(str(item)):
                        self._mark_entry_invalid(entry)
                        self._summary_collector.add_error(
                            "Depends on entry pattern mismatch found {0} must \
                            be <string>/<string>:<string>, ".format(str(item)))
...
class IndexProjectsValidator(IndexValidator):
    ...
    def update_git_url(repo_dump, git_url, git_branch):
        ...
        cmd = "git branch -r | grep -v '\->' | while read remote; \
        do git branch --track \"${remote#origin/}\" \
    ...
    def run(self):
        ...
        for entry in self._index_data:
            ...
            clone_path = self.update_git_url(
                self._context.environment.repo_dump,
                entry["git-url"], entry["git-branch"])

            if clone_path is None:
                self._mark_entry_invalid(entry)
                self._summary_collector.add_error(
                    "Could not clone specified git-url or \
            ...
            container_name = entry["app-id"] + "/" + \
                entry["job-id"] + ":" + str(entry["desired-tag"])
            ...
            if constants.BUILD_CONTEXT in entry:
                ...
                if build_context and not path.exists(
            ...
            # TODO : Ideally, if prebuild is not in entry, it wont reach here
            # and this should happen if it is not None
            prebuild_exists = False
            if constants.PREBUILD_SCRIPT in entry \
                    and constants.PREBUILD_CONTEXT in entry:
                ...
                if (prebuild_script and not path.exists(
                    path.join(git_path, prebuild_script))
            ...
            if not prebuild_exists and not path.exists(
                    git_path + "/" + entry["target-file"]):
                self._mark_entry_invalid(entry)
                self._summary_collector.add_error(
    ...
    def _cccp_yml_check(self, git_path, cccp_yaml_path, entry):
        ...
        if "test-skip" in cccp_yaml:
            ...
            try:
                if value and (value is True or value is False):
                    pass
                if value is None:
                    self._summary_collector.add_warning("Optional test-skip \
                    is set None, which means its value will be ignored")
        ...
        if "test-script" in cccp_yaml:
            ...
            if value is None:
                self._summary_collector.add_warning("Optional test-script has \
                    a value of None, which means it will be ignored")
        ...
        if "build-script" in cccp_yaml:
            ...
            if value is None:
                self._summary_collector.add_warning(
                    "Optional build-script has has a value None, which means \
                    it will be ignored")
        ...
        if "delivery-script" in cccp_yaml:
            ...
            if value is None:
                self._summary_collector.add_warning(
                    "Optional delivery script has value None, which means it \
                    will be ignored")
...
class DependencyValidationUpdater(IndexValidator):
    """
    This script reads the index file and updates the dependency graph,
    created nodes and edges, for containers and
    ...
    def run(self):
        ...
        for entry in self._index_data:
            ...
            primary_container_name = str(
                entry["app-id"]) + "/" + str(
                    entry["job-id"]) + ":" + str(entry["desired-tag"])
            # Add the container to dependency graph (if it does not already
            # exist)
            self._context.dependency_validator.dependency_graph.add_container(
                primary_container_name, from_index=True)
            ...
            if entry["depends-on"]:
                ...
                for item in value:
                    if ":" not in item:
                        item += ":latest"
                    # Add the dependent container to dependency graph, if it
                    # does not already exist
                    self._context.dependency_validator.dependency_graph.add_container(
                        str(item), from_index=True)
                    # Ensure that the dependency from current depends-on
                    # container and the current container is
                    # established
                    self._context.dependency_validator.dependency_graph.add_dependency(
                        str(item), primary_container_name)
            ...
            if ":" in target_file_dir:
                # If the git-url containers :, then we dont need the protocol
                # part, so just get the uri
                target_file_dir = target_file_dir.split(":")[1]
            # The final git-path would be path where all repos are dumped +
            # the git-url part + git-path
            # Example : repo_dump = /mydir, git-url =
            # https://github.com/user/repo, git-path= /mydir
            # then final path = /mydir/github.com/user/repo/mydir
            target_file_dir = self._context.environment.repo_dump + "/" +\
                target_file_dir
            target_file = target_file_dir + "/" + \
                git_path + "/" + entry["target-file"]
            get_back = getcwd()
                ...
                with open(target_file, "r") as f:
                    for line in f:
                        ...
                        if ln.startswith("FROM"):
                            base_image = ln.split()[1]
                            break
            except Exception as e:
                print e
            if base_image:
                self._context.dependency_validator.dependency_graph.add_container(
                    container_name=base_image, from_target_file=True)
                self._context.dependency_validator.dependency_graph.add_dependency(
                    base_image, primary_container_name)
        return True


class LightWeightValidator(IndexValidator):
    """
    Light weight validator does minimum validation, and focuses mostly
    on building dependency graph
    """
    ...
    def run(self):
        for entry in self._index_data:
            self._summary_collector = SummaryCollector(
                self._context, self._file_name, entry)
            if "git-url" not in entry or "git-branch" not in \
                entry or "git-path" not in entry or "target-file" not in \
                    entry:
                self._summary_collector.add_error(
            ...
            if not clone_location:
                self._summary_collector.add_error(
                    "Unable to clone specified git-url or find specified \
                    git-branch")
                self._success = False
                continue
            validation_path = clone_location + "/" + \
                str(entry["git-path"]) + "/" + entry["target-file"]
        ...
        if self._success:
            DependencyValidationUpdater(
                context=self._context, index_file=self._index_file).run()
