...
def main():
    ...
    for key, value in cccp_yml.iteritems():
        ...
        if(key == "build-script"):
            ...
            add_build_steps(target_file_path, "RUN ln -s " +
                            build_script_path + " /usr/bin/build_script")
            got_build_script = 1
        ...
        if(key == "test-script"):
            ...
            if not test_skip:
                ...
                add_build_steps(target_file_path, "RUN ln -s " +
                                test_script_path + " /usr/bin/test_script")
                got_test_script = 1
        ...
        if(key == "delivery-script"):
            ...
            add_build_steps(target_file_path, "RUN ln -s " +
                            delivery_script_path + " /usr/bin/delivery_script")
            got_delivery_script = 1

    if(got_build_script == 0):
        echo_scripts("/build_script", "Build ")
        add_build_steps(target_file_path,
