...
CWD=`dirname $0`
PN=`python -c "import hashlib;print hashlib.sha224('${APPID}-${JOBID}-${DESIRED_TAG}').hexdigest()"`
NS="--namespace ${PN}"
...
_oc ${NS} process ${PN} -v SOURCE_REPOSITORY_URL=${REPO} \
                        -v REPO_BRANCH=${REPO_BRANCH} \
                        -v APPID=${APPID} \
                        -v JOBID=${JOBID} \
                        -v REPO_BUILD_PATH=${REPO_BUILD_PATH} \
                        -v TARGET_FILE=${TARGET_FILE} \
                        -v NOTIFY_EMAIL=${NOTIFY_EMAIL} \
                        -v TEST_TAG=${TEST_TAG} \
                        -v DESIRED_TAG=${DESIRED_TAG} | _oc ${NS} create -f -
...
python $CWD/send_build_request.py ${APPID} ${JOBID} ${DESIRED_TAG} ${REPO_BRANCH} ${REPO_BUILD_PATH} ${TARGET_FILE} ${NOTIFY_EMAIL} ${DEPENDS_ON} ${LOGS_DIR} ${TEST_TAG}
